...
watch(gastos, () =>{
    const totalGastado = gastos.value.reduce((acc, gasto) => acc + gasto.cantidad, 0)
...
onMounted(() =>{
    ...
    if(presupuestoLocal){
        presupuesto.value = parseInt(presupuestoLocal)
        disponible.value = parseInt(presupuestoLocal)
    }
...
const eliminarGasto = (id) =>{
    if(confirm('¿Estás seguro que desea eliminar este gasto?')){
...
const resetApp = () =>{
    if(confirm('¿Estás seguro que deseas resetear la app?')){
        localStorage.clear()
        presupuesto.value = 0
        disponible.value = 0
        gastado.value = 0
